...
- Reads: every open sheet (standalone `/charsheet`, `campaign-sheet?edit=1`, VTT embedded, builder `PlayerSheetPanel`, read-only viewer) = one full GET on mount (cloud: `useCloudCharacterRow`, no Realtime, `replaceRow`/`markDeleted`; local: store) + vitals from the digest (parent `liveDigest` in VTT/builder, else own `useCharacterDigests({characterIds:[id], deriveMaxHp:false})`; max HP = local base + digest `maxHPBonus`) + content via `shared/cloud/sync/useCharacterSheetRevision.js` on `character_sheet_revisions` (17; `characters.sheet_revision` set in the 13 trigger: +1 only when name/data outside vitals+runtime-only keys changes). Newer revision + clean → `getCloudCharacter(id,{reason:'structural-refresh'})` → cloud: parent `onSheetRow`; local: store with emit:false + `markCharacterSynced` + reload. Saves conditional on `sheet_revision` (`updateCloudCharacterData(...,{expectedSheetRevision})`, `pushCharacter`/`updateForeignCharacter` use local meta `gb:char-sync:<id>` = sheetRevision/editVersion/syncedVersion); `SHEET_CONFLICT` → `conflict-refresh` read → banner (Load updated version / Keep my changes), no merge. Dirty: cloud = `pendingCloudSaveRef`/`cloudSaveInFlightRef`; local = `isCloudSyncBusy` (`cloudSyncState.js`, states scheduled/syncing/synced/error/conflict via `gb:cloud-sync`) or `hasUnsyncedLocalChanges`. Own echo via `CHARACTER_SHEET_SAVED_EVENT`/`noteSaved`; held revisions settle after saves. Recovery only SUBSCRIBED/focus/visibility/online — never a periodic timer. Diagnostics: `syncDiagnostics.js` `tagSync` → `__gbRt` `tags` (initial-load, structural-refresh, conflict-refresh, health-fallback, pull, sheet_revision check). No client subscribes to `characters`. Editable sheets deliberately do NOT merge; user decision 2026-09-24 (vitals-only) superseded 2026-09-25 by this content sync. Everyone who only needs roster/vitals — the battle map (`pages/vtt/scene/useCampaignRoster.js`) and the encounter builder (`pages/encounterbuilder/campaign/useCharacterVitalSync.js`, one channel for all linked PCs) — follows `character_digests` through `shared/cloud/sync/useCharacterDigests.js`, never `characters`. A digest (`shared/campaign/characterDigest.js`) carries name/owner/class/colour/portrait, the synced vitals and `hpBasis`; base max HP is derived in JS (`readBaseMaxHp`: runtime adapters + `calcMaxHP`, as `commandCharacterVitals` does) and a full sheet is read only when `hpBasis` changes; `maxHP = max(1, base + maxHPBonus)`. Health-command answers (`publishCharacterRow`) preview a digest locally; the database digest of the same `row_revision` settles it. Received rows never trigger a write.
...
- `useCloudCharacterRow` reads a cloud sheet once; `CharacterSheet` follows vitals (digest) and content (`useCharacterSheetRevision`) itself and hands newer rows back via `onSheetRow`.
- Runtime-only character fields (`shared/character/profile/runtimeFields.js`, now `optionalFeatureEntries` = optional-feature catalog, same for every PC, rebuilt on sheet open): never persisted. `stripRuntimeOnlyCharacterFields` at store save/load, every `characters.data` write + pull (`cloudCharacters.js` `sheetData`), builder `buildSheetCharacter`; the standalone sheet re-attaches them after `storePatchCharacter` (`withRuntimeOnlyCharacterFields`). DB: `character_runtime_only_keys()` stripped on INSERT/UPDATE (13), ignored by `hpBasis` (16); one-off cleanup `supabase/maintenance/remove_optional_feature_entries.sql` (not in the numbered sequence). A new runtime field must be added to both lists (test compares them).
...
- SQL files in `react-app/supabase/` carry their run order in a two-digit prefix (`01_schema.sql` … `16_character_digests.sql`); run them in name order, all are re-runnable (purpose of each in `CLOUD_SETUP.md`). A new script takes the next number; `tests/logic/shared/cloud/schema-order.sql.test.js` fails on an unnumbered or duplicate-numbered file and applies the whole sequence twice. The player mark RPCs (`set_token_conditions/effects`) are defined once in `06_vtt.sql` and find `forward_token_marks` at call time, so re-run order is free. `patch_character_data` is retired (dropped by `04_characters_realtime.sql`, which also keeps `characters` out of the `supabase_realtime` publication: `characters` = REST/RPC only; Realtime only on `character_digests` (vitals/roster/hpBasis) and `character_sheet_revisions` (content-changed signal); `tests/logic/shared/cloud/realtime-subscriptions.test.js` fails on a `postgres_changes` subscription to `characters`, a non-literal table, or an unpublished one). `uuid_or_null` is intentionally defined in both `05_character_art.sql` and `06_vtt.sql` so each stands alone.
