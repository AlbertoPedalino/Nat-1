...
import {
  ...
  simulateThrow,
  thrownDieSize,
} from '../../../shared/vtt/rolls/dicePhysics.js';
...
function DiceThrow({ roll, at, table, startedAt, onSettled, centered }) {
  ...
  const thrown = useMemo(() => {
    ...
    const size = thrownDieSize(dice.length);
    const { frames, frameMs, results } = simulateThrow(dice, roll.id, { size });
    return {
      frames,
      frameMs,
      size,
    ...
    <Box sx={{ ...anchorSx, left: x, top: y }}>
      {thrown.dice.map((die, index) => (
        <Box
          ...
          sx={{
            ...bodySx,
            marginLeft: `${-thrown.size / 2}px`,
            marginTop: `${-thrown.size / 2}px`,
          }}
        >
